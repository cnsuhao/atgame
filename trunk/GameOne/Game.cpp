...
void MyMdxNode::UpdataAninData(float elapsedTime, float startTime, float endTime)
{
    ...
    for (int i = 0; i < numberChild; i++)
    {
        ...
        if (child)
        {
            MyMdxNode::Update(child, elapsedTime, startTime, endTime);
        }
    }
    int time = static_cast<int>(elapsedTime);
    atgJointAnimData::UpdateAnim(this, this, time, startTime, endTime);
}
...
bool MyMdxMaterial::Load(const char* fileName)
{
    ...
    for (int i = 0 ; i < material->numberLayers; i++)
    {
		...
		JPEG_Loader::Load(&image, texturePath.c_str(), false, IsOpenGLGraph() ? CO_RGBA : CO_ARGB);
...
bool Game::OnInitialize()
{
    ...
    SetRandomSeed((uint32)GetAbsoluteMsecTime());
    g_palyIndex = 8;//Random(0, g_Animation->GetNumberAnimation());
    ...
    Texture* tex = (Texture*)System::getSingletonPtr()->getRenderer()->createTexture("ui/ui1.jpg");
...
void Game::OnFrame()
{
	...
	if (_mainScript)
	{
		_mainScript->OnUpdate(elapsedTime);
	}
...
void Game::LoadConfig()
{
	...
	_mainScript = new ScriptObject();
	_mainScript->LoadFile("scripts/obj002.lua");
}
